...
def calcular_puntos(numero_lanzamiento, dados, juego):
    ...
    if juego == "escalera":
        ...
        if dados == [1, 2, 3, 4, 5] or dados == [2, 3, 4, 5, 6]:
            ...
            if  dados == [1, 2, 3, 4, 5]:
                juego == "escalera_menor"
                puntos = 20
            if  dados == [2, 3, 4, 5, 6]:
                juego == "escalera_mayor"
                puntos = 20 
            if numero_lanzamiento == 1 and dados == [2, 3, 4, 5, 6]:
                puntos = 25
                juego = "escalera_mayor_servida"
    ...
    if juego == "generala":
        ...
        for i in range (len(dados)):
            if buscar_repetido_full(dados, repetidos , 3) and buscar_repetido_full(dados, repetidos , 2):
                ...
                if numero_lanzamiento == 1:
                    puntos += 5
                    if buscar_repetido_full(dados, repetidos , 1):
                        puntos = 0
